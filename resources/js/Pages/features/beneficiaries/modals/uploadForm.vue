<template>
    <div>
        <b-modal v-model="modalItemValue" hide-footer scrollable centered size="lg" title="Upload List of Beneficiaries"
            @close="closeModal">
            <div class="tailwind-classes">
                <Vueform ref="formInfo" v-model="formDetails" sync>
                    <TextElement :disabled="isDisabled" :floating="false" name="title" size="sm" label="Title"
                        placeholder="title" :columns="{
                        ...
                        <template #between="scope">
                            ...
                            <div v-for="error in v$['title'].$errors" :key="error.$uid + `title`">
...
export default {
    ...
    setup(props, ctx) {
        ...
        const uploadFile = async () => {
            ...
            await RequestHelper.postRequest(FINANCE_URL +"/api/beneficiaries/upload-beneficiaries", formDetails.value, {}, props.session?.api_token)
                .then(({ data }) => {
                    console.log({ data });
                    Swal.close();
